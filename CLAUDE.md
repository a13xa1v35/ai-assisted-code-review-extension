...
A VS Code extension ("Human Review") that renders AI-generated semantic PR reviews. Two decoupled parts:
...
- `GitContentProvider` — custom `TextDocumentContentProvider` for the `human-review-git:` URI scheme. Resolves file content at a git ref via `git show "<ref>:<path>"`.
- `activate()` — registers the git provider, sidebar webview, commands (`humanReview.open`, `humanReview.openGroup`), and a file watcher for `**/human-review.json`.
...
The agent writes `./human-review.json` at the workspace root. The extension watches for it via `FileSystemWatcher`. Schema is defined in `src/types.ts` and mirrored in `skill/review.md`.

## Key Patterns

- Diff viewing uses a custom URI scheme (`human-review-git:`) registered as a `TextDocumentContentProvider`, not temp files.
